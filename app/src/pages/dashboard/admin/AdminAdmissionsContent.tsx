...
const AdminAdmissionsContent: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      const response = await api.get('/api/pages/admissions');
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await api.post('/api/pages/admissions', { hero, content });
      const result = response.data;
      ...
      <div className="section-content">
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-3xl font-bold">Modifier - Page Admissions</h1>
            <button
              onClick={handleSave}
              disabled={saving}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="input-field w-full"
                  placeholder="/admissions-hero.jpg"
                />
              </div>
            </div>
          </div>
              ...
              {Object.entries(content).map(([key, value]) => (
                <div key={key}>
                  <label className="block text-sm font-medium mb-2 capitalize">
                    {key === 'requirements' ? 'Critères d\'admission' :
                     key === 'process' ? 'Processus d\'admission' :
                     key === 'timeline' ? 'Calendrier d\'admission' :
                     key === 'contact' ? 'Informations de contact' : key}
                  </label>
                  <textarea
                    value={value || ''}
                    onChange={(e) =>
